...
function CsvImportBlock({
  ...
  return (
    <div className="space-y-3 rounded-xl border border-violet-200 dark:border-slate-700 bg-fuchsia-50 dark:bg-slate-900 p-4 shadow-sm dark:shadow-slate-950/50">
      <div>
        <h2 className="text-lg font-semibold">{title}</h2>
        <p className="text-sm text-violet-700 dark:text-slate-400">{hint}</p>
      </div>

      <label className="block text-sm">
        <span className="text-violet-700 dark:text-slate-400">Calendar year for this tab</span>
        <input
          type="number"
          value={year}
      ...
      <input
        ...
        className="block w-full text-sm text-violet-800 dark:text-slate-400 file:mr-3 file:rounded-lg file:border-0 file:bg-violet-100 dark:bg-slate-800 file:px-3 file:py-2 file:text-sm file:font-medium"
      />
      ...
      <textarea
        ...
        rows={6}
        className="w-full rounded-lg border border-violet-300 dark:border-slate-600 px-3 py-2 text-sm font-mono"
      />
      ...
      {message && (
        <p className={`text-sm ${message.includes("failed") || message.includes("No ") ? "text-red-600" : "text-violet-800 dark:text-slate-400"}`}>
          {message}
        </p>
      )}
