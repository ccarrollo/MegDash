...
export function StopCard({
  ...
  return (
    <article className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm dark:shadow-slate-950/50">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            ...
            {!isLunch && !hasCustomTime && editingTime && (
              <button
                ...
                className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 hover:underline"
              >
                Cancel
              </button>
            )}
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 dark:text-slate-400">{stop.facilityName}</p>
        </div>
        <span className="shrink-0 text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">
          {ZONE_LABELS[stop.zone]}
        </span>
      </div>

      <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400">{stop.reason}</p>
