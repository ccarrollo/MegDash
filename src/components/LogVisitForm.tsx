...
import { DETAIL_GROUPS } from "@/lib/visitDetailOptions";
...
export function LogVisitForm({ doctorId }: { doctorId: string }) {
  ...
  async function log() {
    ...
    try {
      ...
      try {
        const key = `meg-field:doctor-note-days:${doctorId}`;
        const raw = localStorage.getItem(key);
        const expanded = new Set<string>(raw ? (JSON.parse(raw) as string[]) : []);
        expanded.add(today);
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-2">
        <label className="flex items-center gap-2 rounded-lg border border-violet-300 dark:border-slate-600 px-3 py-2 text-sm text-violet-900 dark:text-slate-300"
        >
          <input
            type="checkbox"
      ...
      {showDetails && (
        <div className="mt-3 space-y-3 rounded-lg border border-violet-200 dark:border-slate-700 p-3">
            ...
            <div key={group.title}>
              ...
              <div className="mt-2 grid gap-2">
                {group.options.map((option) => (
                  <label
                    key={option.value}
                    className="flex items-center gap-2 text-sm text-violet-900 dark:text-slate-300"
                  >
                    <input
                      type="checkbox"
